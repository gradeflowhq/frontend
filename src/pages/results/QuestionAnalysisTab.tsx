...
const QuestionAnalysisTab: React.FC<Props> = ({ items, questionIds }) => {
  ...
  const perQuestion = useMemo(() => {
    return questionIds.map((qid) => {
      ...
      // Build histogram data for Recharts (12 bins, full-width container)
      const bins = 12;
          ...
          <div key={q.qid} className="card bg-base-100 border border-base-300 shadow-xs">
            <div className="card-body space-y-3">
              ...
              <div className="divide-y divide-base-300">
                <Line label="Attempts" right={
                  <>
                    <span className="font-semibold">{q.attempts}</span>
                    <span className="opacity-60 ml-1">/ {totalStudents}</span>
                  </>
                } />
                ...
                <Line label="Mean">
                  <span className="font-mono">{q.stats.mean.toFixed(2)}</span>
                </Line>

                <Line label="Median">
                  <span className="font-mono">{q.stats.q2.toFixed(2)}</span>
                </Line>

                <Line label="Std dev">
                  <span className="font-mono">{q.stats.stdev.toFixed(2)}</span>
                </Line>
                ...
                <Line label="Adjustments">
                  <span className="font-mono">{q.adjustmentCount}</span>
                </Line>

                <Line label="Avg Δ points">
                  <span className="font-mono">{q.adjustmentAvg.toFixed(2)}</span>
                </Line>
              </div>

              {/* Distribution (Recharts full-width container) */}
              <div className="pt-2">
